...
export default function PlaylistComponent (props: any) {
    ...
    function ComponentPlaylist({ track }: any = []) {
        console.log('ComponentPlaylist', track)
        return (
            <>
           { track.length > 0 ? playlist?.tracks.map((track: any, i: any) => (
                <div key={track?.name} className={"h-24 w-full bg-white mt-10 px-6 rounded-3xl flex shadow-[0_25px_40px_0_rgba(0,0,0,0.02)] justify-center items-center " + (trackContext.track.id == track.id ? style.track__selected : null)}>
                    ...
                    <div className="w-3/5 h-4/5 pr-10">
                        <p className="text-[#2E3271] font-normal text-base">{track.name}</p>
                        <p className="text-[#7c8dfecc] text-sm">{track.artists[0]?.name}</p>
                    </div>
                    <div className="w-20 h-12 rounded-xl justify-center items-center gap-3 flex shadow-[0_15px_30px_0_rgba(0,0,0,0.06)]">
                        <div>
                            <button onClick={() => handleTrackSelected(track)}>
                        ...
                        <p className="text-xs text-[#313D58] font-extralight">{track.popularity}</p>
                    </div> 
                </div>
            )) : <p>sem lista</p>}
            </>
    ...
    return (
            ...
            <div className={"w-full h-full bg-[#ffffffcc] rounded-b-3xl px-5 overflow-y-scroll " + style.custom_overflow}>
                {
                    playlist?.tracks?.map((track: any, i: any) => (
                        <div key={track?.name} className={"h-24 w-full bg-white mt-10 px-6 rounded-3xl flex shadow-[0_25px_40px_0_rgba(0,0,0,0.02)] justify-center items-center " + (trackContext.track.id == track.id ? style.track__selected : null)}>
                             <div className="h-4/5 shadow-[0_15px_30px_0_rgba(0,0,0,0.16)] mr-10"> 
                                <Image
                                className="rounded-3xl"
